...
class SimulationManager(QObject):
    ...
    def __init__(self):
        ...
        # Set to true to stop simulation thread after it finishes the iteration it is on
        self.threadStopped = False

    def setPreferences(self, preferences):
        self.preferences = preferences

    # Show sets if the results will be reported on newSimulationResult and shown in UI
    def runSimulation(self, motor, show=True):
    ...
    def _simThread(self, show):
        ...
        print('dikobraz: uilib+simulManager+ukladani do output_vals.txt') # pridala jsem
        if simRes.success and show:
            simRes.export_znalosti()  # pridala jsem
