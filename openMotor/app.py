...
class App(QApplication):
    def __init__(self, args):
        ...
        # true or false podle vstupnich argumentu
        ...
        # nacitani startUpFile typu .ric, kde jsou popsany parametry motoru+paliv+slozeni
        self.fileManager = uilib.fileManager.FileManager(self)
        startupFileLoaded = False
        if len(args) > 1 and args[-1][0] != '-':
            # poslední argument má být startUpFile
            startupFileLoaded = self.fileManager.load(args[-1])
        self.propellantManager.updated.connect(self.fileManager.updatePropellant)

        # ToolManager je nabidka s 3 moznostmi: SET, OPTIMIZE a DESIGN
        # obsahuje i properties setPreferences(self, pref)
        #                       updateMotor(self, motor)
        #                       requestSimulation(self)
            ...
            if len(args) < 3:
                ...
                for alert in simulationResult.alerts:
                    print('{} ({}, {}): {}'.format(motorlib.simResult.alertLevelNames[alert.level],
                        ...
                        alert.description))
                print()
                simulationResult.export_znalosti()#pribdala jsem
                if '-o' in args:
                    # pousteni aplikace s udanym souborem, kam zapisovat
                ...
                #else:
                    # jinak se soubor vypise, kam je zrovna mozne (do terminalu)
                    # print(simulationResult.getCSV(self.preferencesManager.preferences))
                    #pass
            sys.exit(0)

        else:
            # otevira okno aplikace s GUI, v argumentech neni -h
