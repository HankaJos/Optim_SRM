...
singleValueChannels = ['time', 'kn', 'pressure',
                       'force', 'volumeLoading', 'exitPressure', 'dThroat']
...
class SimulationResult():
    ...
    def getIdealThrustCoefficient(self):
        ...
        _, _, gamma, _, _ = self.motor.propellant.getCombustionProperties(
            chamberPres)
                    ...
                    if self.channels[chan].valueType in (float, int):
                        ...
                        for gid, grainVal in enumerate(self.channels[chan].getPoint(ind)):
                            if gid not in excludeGrains:
                                conv = round(units.convert(
                                    grainVal, self.channels[chan].unit, outUnits[chan]), places)
                                out += str(conv) + ','
    ...
    def export_znalosti(self, path='output_vals.txt'):

        data = [['Burn_Time(s)', self.getBurnTime()],
                ['Initial_Kn', self.getInitialKN()],
                ['Peak_Kn', self.getPeakKN()],
                ['Avg_Chamber_Pressure(Pa)', self.getAveragePressure()],
                ['Max_Chamber_Pressure(Pa)', self.getMaxPressure()],
                ['Impulse', self.getImpulse()],
                ['ISP', self.getISP()],
                ['Port/throat', self.getPortRatio()],
                ['Peak_Mass_Flux', self.getPeakMassFlux()],
                ['Delivered_thrust_coefficient',
                    self.getAdjustedThrustCoefficient()],
                ['Ideal_thrust_coefficient', self.getIdealThrustCoefficient()],
                ['Prop_mass', self.getPropellantMass()],
                ['Prop_length', self.getPropellantLength()],
                ['Volume_loading', self.getVolumeLoading()]]

        # Ensure directory exists
        os.makedirs("tmp", exist_ok=True)

        with open(os.path.join("tmp", path), "w") as file:
